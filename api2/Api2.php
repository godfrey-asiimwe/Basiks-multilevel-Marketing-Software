		...
		switch($_GET['apicall']){
			...
			case 'signup':
				if(isTheseParametersAvailable(array('username','email','password','gender'))){
					...
					if($stmt->num_rows > 0){
						...
						$stmt = $conn->prepare("INSERT INTO accounts (username, email, password, gender) VALUES (?, ?, ?, ?)");
						$stmt->bind_param("ssss", $username, $email, $password, $gender);

						if($stmt->execute()){
							$stmt = $conn->prepare("SELECT id, id, username, email, gender FROM accounts WHERE username = ?"); 
			...
			case 'login':
				...
				if(isTheseParametersAvailable(array('email', 'password'))){
					...
					$stmt = $conn->prepare("SELECT id, username, email, gender FROM accounts WHERE email = ? AND password = ?");
