          ...
          <div class="p-4 m-3">
            ...
            <?php
              ...
              if(isset($_POST['submit'])){
                ...
                if ( !isset($_POST['stockist_id']) ) {
                  ...
                  exit('Please fill code fields!');
                }

                // Prepare our SQL, preparing the SQL statement will prevent SQL injection.
                if ($stmt = $con->prepare('SELECT id, stockist_id FROM Stockist WHERE stockist_id = ?')) {
                  // Bind parameters (s = string, i = int, b = blob, etc), in our case the username is a string so we use "s"
                  $stmt->bind_param('s', $_POST['stockist_id']);
                  $stmt->execute();
                  // Store the result so we can check if the account exists in the database.
                  $stmt->store_result();

                  if ($stmt->num_rows > 0) {
                  $stmt->bind_result($id, $stockist_id);
                  $stmt->fetch();
                  // Account exists, now we verify the password.
                  // Note: remember to use password_hash in your registration file to store the hashed passwords.
                  if ($_POST['stockist_id'] === $stockist_id) {
            ...
            <form method="POST" action="<?php echo $_SERVER['SCRIPT_NAME']?>" class="needs-validation" novalidate="">
              <div class="form-group">
                ...
                <input id="stockist_id" type="password" class="form-control" name="stockist_id" tabindex="2" required>
          ...
          <div class="absolute-bottom-left index-2">
            <div class="text-light p-5 pb-2">
              <div class="mb-5 pb-3">
                ...
                <h5 class="font-weight-normal text-muted-transparent">East Africa</h5>
              </div>
            </div>
          </div>
        </div>
